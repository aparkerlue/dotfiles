...
IFS='
'
for vardef in $(generate_env); do
    export "$vardef"
done
IFS="$ifs_orig"
unset -v ifs_orig vardef
unset -f generate_env
...
prepend_to_path() {
    ...
    if [ $? -eq 0 ] && [ -d "$dir" ] && ! path_contains "$dir"; then
        echo "${dir}:${PATH}"
        return 0
    else
        echo "${PATH}"
...
PATH=$(prepend_to_path $npm_packages/bin)
MANPATH="$npm_packages/share/man:${MANPATH}"
NODE_PATH="$npm_packages/lib/node_modules"
unset -v npm_packages

PATH=$(prepend_to_path $HOME/.poetry/bin)
PATH=$(prepend_to_path $HOME/.cargo/bin)
PATH=$(prepend_to_path $HOME/.local/bin)
PATH=$(prepend_to_path $HOME/bin)

unset -f path_contains prepend_to_path
